...
TK_ENV_VARS="FONTCONFIG_PATH=\$CONDA_PREFIX/etc/fonts"
...
if [[ $1 == "rebuild" ]]; then
	...
	bin/micromamba run -r conda -p conda/envs/linux env $TK_ENV_VARS pyinstaller --noconfirm --onedir --collect-all customtkinter --collect-all jinja2 --collect-all psutil --add-data './koboldcpp.py:.' --add-data './json_to_gbnf.py:.' --version-file './version.txt' --clean --console koboldcpp.py -n "koboldcpp-launcher"
	if [ "$KCPP_CUDA" = "rocm" ]; then
		if [ ! -n "$ROCM_PATH" ]; then
			ROCM_PATH=/opt/rocm
		fi
